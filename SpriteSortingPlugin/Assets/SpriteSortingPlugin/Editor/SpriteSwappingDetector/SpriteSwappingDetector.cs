...
using SpriteSortingPlugin.SortingGeneration.Criteria;
using SpriteSortingPlugin.SortingGeneration.Data;
using SpriteSortingPlugin.SpriteSwappingDetector.Logging;
using SpriteSortingPlugin.SpriteSwappingDetector.UI;
using SpriteSortingPlugin.SpriteSwappingDetector.UI.OverlappingSprites;
...
namespace SpriteSortingPlugin.SpriteSwappingDetector
{
    public class SpriteSwappingDetector : EditorWindow
    {
        ...
        private SortingGenerator sortingGenerator;
            ...
            using (new EditorGUI.DisabledScope(!isUsingSpriteData))
            {
                using (new EditorGUILayout.VerticalScope(Styling.HelpBoxStyle))
                {
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        {
                            var spriteAlphaEditorWindow = GetWindow<SpriteSwappingDataAnalysis.UI.SpriteSwappingDataAnalysis>();
       ...
       //This issue is already fixed but was reverted due to no modifications are allowed after the survey was published for the master thesis.
        ...
        {
            sortingGenerator = new SortingGenerator();
            ...
            foreach (var sortingCriterion in autoSortingOptionsUI.GetActiveSortingCriteria())
            {
                ...
                {
                    sortingGenerator.SetContainmentCriterion(containmentSortingCriterionData);
                    continue;
                }

                sortingGenerator.AddSortingCriterion(sortingCriterion);
            }
        }
